...
function Index() {
    ...
    const { data: users, mutate } = useSWR(process.env.API + '/api/headmaster/user/index?page=' + (router.query.page ? router.query.page : '') + '&search=' + (router.query.search ? router.query.search : ''), fetcher);
    const handlePagination = (index) => {
        setPageIndex(index)
        router.push({
            pathname: '/headmaster/teachers',
    ...
    const doSearch = (event) => {
        ...
        filterTimeout = setTimeout(() => {
            router.push({
                pathname: '/headmaster/teachers',
                query: { ...router.query, page: 1, search: event }
            },
                undefined,
                {}
            )
    ...
    if (!users) {
        return <>
            <PageLayout title={'Daftar TPK'} hasBackUrl={true} backUrl="/headmaster">
                            ...
                            users.data?.data?.map((user, index) => {
                                return (
                                    <tr className="hover" key={index}>
                                        ...
                                        <th>
                                            <div className="flex gap-2">
                                                <Link href={'/headmaster/teachers/' + user.id}>
