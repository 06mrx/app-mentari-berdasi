...
export default function Detail() {
    ...
    let id = router.query.id_number;
    if (router.isReady) {
        id = router.query.id_number
    }
    const { data: student, error } = useSWR(process.env.API + '/api/parent/student-personal-data/show/' + id, id ? fetcher : null)
    const { data: chartData } = useSWR(process.env.API + '/api/parent/student-personal-data/chart/' + id, id ? fetcher : null)
    // const { data: healthActions, mutate } = useSWR(process.env.API + '/api/parent/student-health-action/list/' + id, id ? fetcher : null)
    // const { data: growthStatus } = useSWR(process.env.API + '/api/parent/student-personal-data/check-status/' + id, id ? fetcher : null)
    ...
    if (!student) return <></>
    return <>
        <GuestLayout title={'Detail Siswa'} hasBackUrl={true} backUrl="/parent">
            <ToastContainer />
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div>
                    <div className="bg-sky-300 rounded-2xl shadow-xl p-3 flex flex-col items-center relative">
                        ...
                        {/* <div class="absolute inset-x-0 bottom-2 text-center bg-sky-300">
                            <Link href={'/leader/students/' + student.data?.id + '/growth'}>
                ...
                <div className=" ">
                    <div className="bg-base-100 rounded-2xl shadow-xl">
                        <p className="font-semibold text-xl md:text-2xl lg:text-2xl p-3">Perkembangan Tinggi Badan 6 Bulan Terakhir</p>
                        <ReactApexChart options={options1} series={series1} type="line" height={280} />
                        <p className="font-semibold text-xl md:text-2xl lg:text-2xl p-3">Perkembangan berat Badan 6 Bulan Terakhir</p>
                        <ReactApexChart options={options2} series={series2} type="line" height={280} />
                        {/* <div class=" inset-x-0 bottom-2 rounded-2xl text-center bg-base-100 mb-5">
                            <Link href={'/leader/students/' + student?.data?.id + '/growth'}>
                                <div className="flex w-full items-center justify-center group">
                                    <span className="font-semibold cursor-pointer group-hover:text-sky-500">
                                        Lihat Selengkapnya
                ...
                <div className="relative bg-base-100 rounded-2xl shadow-xl p-2 overflow-auto">
                    ...
                    {
                        student.data?.student_health_actions?.map((healthAction, index) => {
                            return (
                    ...
                    {
                        student.data?.student_health_actions?.lenght == 0 ? <p className="mt-5 mb-10 text-center opacity-60">Tidak ada data</p> : ''
                    }

                    {/* <div class="absolute inset-x-0 bottom-2 text-center bg-base-100">
                        <Link href={'/leader/students/' + student.data?.id + '/growth-intervention'}>
                            <div className="flex w-full items-center justify-center group">
                                <span className="font-semibold cursor-pointer group-hover:text-sky-500">
                                    Lihat Selengkapnya
                                </span>
