...
function Index() {
    ...
    const { data: users, mutate } = useSWR(process.env.API + '/api/administrator/user/teacher?page=' + (router.query.page ? router.query.page : '') + '&search=' + (router.query.search ? router.query.search : '') + '&unit_id=' + (router.query.unit_id ? router.query.unit_id : ''), fetcher);
    const { data: units } = useSWR(process.env.API + '/api/reference/work_unit/list', fetcher);
    
    const handlePagination = (index) => {
        setPageIndex(index)
        router.push({
            pathname: '/leader/teachers',
    ...
    if (!users || !units) {
        return <>
            <PageLayout title={'Daftar Guru'} hasBackUrl={true} backUrl="/leader">
        ...
        <PageLayout title={'Daftar Guru'} hasBackUrl={true} backUrl="/leader">
            <div className="flex flex-col md:flex-row lg:flex-row w-full  justify-center items-center gap-3">
                ...
                <div className="flex w-full lg:w-auto lg:justify-start justify-center gap-3">
                    <select defaultValue={router.query.unit_id ? router.query.unit_id : ''} onChange={(event) => doFilter(event.target.value)} className="select select-bordered select-sm w-full max-w-xs">
                            ...
                            users.data?.data?.map((user, index) => {
                                return (
                                    <tr className="hover" key={index}>
                                            ...
                                            <div className="flex items-center space-x-3">
                                                ...
                                                <div>
                                                    <div className="font-bold">
                                                        <Link href={'/leader/teachers/' + user.id} className="text-primary underline">
                                        ...
                                        <th>
                                            <div className="flex gap-2">
                                                <Link href={'/leader/teachers/' + user.id}>
