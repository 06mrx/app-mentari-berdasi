...
function Index() {
    ...
    const { data: students, mutate } = useSWR(process.env.API + '/api/administrator/student-personal-data/index?page=' + (router.query.page ? router.query.page : '') + '&search=' + (router.query.search ? router.query.search : '') + '&unit_id=' + (router.query.unit_id ? router.query.unit_id : ''), fetcher);
    const { data: workUnits } = useSWR(process.env.API + '/api/reference/work_unit/list', fetcher);
    let filterTimeout;
    const doSearch = (event) => {
        clearTimeout(filterTimeout)
        filterTimeout = setTimeout(() => {
            router.push({
                pathname: '/education-authorities/students',
    ...
    return <>
        <PageLayout title={'Daftar Siswa PAUD'} hasBackUrl={true} backUrl="/education-authorities">
            <div className="flex flex-col md:flex-row lg:flex-row w-full  justify-center items-center gap-3">
                <div className="flex flex-col w-full md:w-6/12 lg:w-6/12">
                ...
                <div className="flex w-full lg:w-auto lg:justify-start justify-center gap-3">
                    <select defaultValue={router.query.unit_id} onChange={(event) => doFilter(event.target.value)} id="unit_id" className="select select-bordered select-sm w-full max-w-xs">
                        <option disabled value={''}>Filter Sekolah</option>
                        {
                            workUnits?.data?.map((workUnit, index) => {
                                return (
                                    <option value={workUnit.id} key={index}>{workUnit.name}</option>
                                )
                            })
                        }
                    </select>
            ...
            <div className="overflow-x-auto w-full mt-3 max-w-screen-2xl mx-auto">
                <div className="flex w-full justify-end">
                    <Link href={'/education-authorities/students/create'}>
                                ...
                                return (
                                    <tr className="hover" key={index}>
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6" viewBox="0 0 24 24"><path fill="#ffb266" d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM21.41 6.34l-3.75-3.75l-2.53 2.54l3.75 3.75l2.53-2.54z"></path></svg>
                                                ...
                                                <Link href={'/education-authorities/students/' + student.id}>
