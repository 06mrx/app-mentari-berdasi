...
export default function Detail() {
    ...
    const { data: student, error } = useSWR(process.env.API + '/api/teacher/student-personal-data/show/' + id, id ? fetcher : null)
    const { data: chartData } = useSWR(process.env.API + '/api/teacher/student-personal-data/chart/' + id, id ? fetcher : null)
    const { data: healthActions, mutate } = useSWR(process.env.API + '/api/administrator/student-health-action/list/' + id, id ? fetcher : null)
    const { data: growthStatus } = useSWR(process.env.API + '/api/teacher/student-personal-data/check-status/' + id, id ? fetcher : null)
    ...
    return <>
        <PageLayout title={'Detail Ibu / Calon Ibu'} hasBackUrl={true} backUrl="/teacher/mother">
            <ToastContainer />
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div>
                    <div className="bg-sky-300 rounded-2xl shadow-xl p-3 flex flex-col items-center relative">
                        ...
                        <div class="absolute inset-x-0 bottom-2 text-center bg-sky-300">
                            <Link href={'/teacher/students/' + student.data?.id + '/growth'}>
                    ...
                    <div className="bg-base-100 rounded-2xl shadow-xl p-3 mt-3">
                        <p className="font-semibold mb-3 text-2xl">Perkembangan Ibu / Calon Ibu</p>
                        <p className="opacity-90 text-sm leading-3">Berdasarkan hasil visitasi terakhir pada tanggal 02-03-2023</p>
                        ...
                        <div className="bg-indigo-300 p-3 rounded-2xl shadow-md mt-3">
                            <div className="flex gap-3">
                                ...
                                <div className="flex  flex-col w-full">
                                    <p className="font-semibold text-2xl">Status Kesehatan</p>
                ...
                <div className="bg-base-100 rounded-2xl shadow-xl relative">
                    <div className="flex gap-3 justify-between items-center pr-3">
                        ...
                        <Link href={'/teacher/students/' + student?.data?.id + '/growth/create'}>
                            <button className="btn btn-info btn-sm">+ Tambah</button>
                        </Link>
                    </div>
                ...
                <div className="relative bg-base-100 rounded-2xl shadow-xl p-2 overflow-auto">
                    ...
                    <div class="absolute inset-x-0 bottom-2 text-center bg-base-100">
                        <Link href={'/teacher/students/' + student.data?.id + '/growth-intervention'}>
                            <div className="flex w-full items-center justify-center group">
                                <span className="font-semibold cursor-pointer group-hover:text-sky-500">
                                    Lihat Selengkapnya
                                </span>
