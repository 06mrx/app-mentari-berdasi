...
export default function Index() {
    ...
    const { data: activities, mutate } = useSWR(process.env.API + '/api/reference/output-unit/index', fetcher)
    ...
    const handleDelete = () => {
        DELETE(process.env.API + '/api/reference/output-unit/destroy/' + selected.id, token)
    }
    const handleSubmit = event => {
        event.preventDefault();
        
        fetch(process.env.API + '/api/reference/output-unit/store/',{
            method: 'POST',
            headers: {
                'authorization': token,
                'Content-Type': 'application/json'
    ...
    const handleEdit = event => {
        ...
        fetch(process.env.API + '/api/reference/output-unit/update/' + selected.id, {
            method: 'POST',
            headers: {
                'authorization': token,
                'Content-Type': 'application/json'
            ...
            body: JSON.stringify({
                ...
                target: formData.target.current.value,
            })
        }).then((res) => res.json()).then((data) => {
            if (!data.success) {
                setErrors(data.data)
    ...
    return <>
        <PageLayout title={'Referensi Luaran TPK'} hasBackUrl={true} backUrl="/administrator">
            <ToastContainer />
            <div className="p-2">
                <div className="hidden md:flex lg:flex justify-end w-full">
                    <label onClick={() => setSelected(null)} htmlFor="add-modal" className="btn btn-info btn-sm">
                        + Tambah
                    </label>
                </div>
                <div className="fixed z-50 md:hidden lg:hidden bottom-10 right-5">
                    <label onClick={() => setSelected(null)} htmlFor="add-modal" className=" btn btn-circle btn-info">
                        +
                    </label>
                </div>
            ...
            <label htmlFor="edit-modal" className="modal cursor-pointer">
                <label className="modal-box relative" htmlFor="">
                    ...
                    <form onSubmit={handleEdit}>
                        ...
                        <div className="form-control w-full">
                            <label className="label">
                                <span className="label-text font-semibold">Target TPK / Bulan (Per orang)</span>
                                <span className="label-text-alt text-red-500 font-semibold">* Wajib</span>
                            </label>
                            <input type="number" placeholder="Masukkan Target" id="name" key={selected?.target} defaultValue={selected?.target} className="input input-sm w-full input-bordered" ref={formData.target} />
                            <label className="label">
                                <span className="text-xs italic text-red-500">{(errors.hasOwnProperty("name")) ? errors.name[0] : ''}</span>
                            </label>
                        </div>
                ...
                <label className="modal-box relative" htmlFor="">
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="form-control w-full">
                            ...
                            <input placeholder="Masukkan Nama" key={2} id="name_add"  className="input input-sm w-full input-bordered" ref={formData.name} />
                            <label className="label">
                                <span className="text-xs italic text-red-500">{(errors.hasOwnProperty("name")) ? errors.name[0] : ''}</span>
                            </label>
                        </div>
                        <div className="form-control w-full">
                            ...
                            <input placeholder="Masukkan target" key={1} type="number" id="target_add"  className="input input-sm w-full input-bordered" ref={formData.target} />
                            <label className="label">
                                <span className="text-xs italic text-red-500">{(errors.hasOwnProperty("target")) ? errors.target[0] : ''}</span>
                            </label>
                        </div>
                    </form>
                    <div className="modal-action">
